...
export class SingleTouchManager {
  ...
  onArrowChange?: (arrow: Arrow) => void;
  onArrowFinish?: (arrow: Arrow) => void;
  onArrowStop?: () => void;
  ...
  useUpdateProps(
    onTouchScreenMove: ((move: Direction) => void) | undefined,
  ...
  useAttachToSvg() {
    useEffect(() => {
      ...
      this.svgRef.current.addEventListener("touchstart", this.onTouchStart, {
        passive: false,
      });
      this.svgRef.current.addEventListener("touchmove", this.onTouchMove, {
        passive: false,
      });
      return () => {
  ...
  onTouchMove = (e: React.TouchEvent<SVGElement> | TouchEvent) => {
    ...
    const changedTouch = Array.from(e.changedTouches).find(
      (changedTouch) => changedTouch.identifier === this.identifier,
    );
    if (!changedTouch) {
      return;
    }
    ...
    const arrow: Arrow = {
      start: {
        ...
        x: changedTouch.pageX - clientRect.left,
        y: changedTouch.pageY - clientRect.top,
      },
    };
    if (!nextMove) {
      if (positionDistance(arrow.start, arrow.end) > 2) {
        this.onArrowChange?.(arrow);
      }
      return;
    }
    if (
      this.restrictTouchScreenMovesTo &&
      !this.restrictTouchScreenMovesTo[nextMove]
    ) {
      return;
    }
    this.onTouchScreenMove?.(nextMove as Direction);
    this.onArrowFinish?.(arrow);
    this.x = changedTouch.pageX;
    this.y = changedTouch.pageY;
  };
